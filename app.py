...
<body><div class='wrap'>
  <h1>Dream Trance MIDI Generator V1.1</h1>
  <p class='sub'>Section-aware uplifting vocal trance generator. Exports full-length aligned MIDI stems plus a combined arrangement MIDI.</p>
  <form method='post' action='/generate'>
    <div class='grid'>
      <label><span>BPM</span><input type='number' name='bpm' min='132' max='142' value='138'></label>
      <label><span>Key Root</span><select name='key_root'>{key_opts}</select></label>
      <label><span>Progression</span><select name='progression'>{prog_opts}</select></label>
      <label><span>Arrangement</span><select name='arrangement'>{arr_opts}</select></label>
      <label><span>Energy</span><select name='energy'>{energy_opts}</select></label>
      <label><span>Vocalist</span><select name='vocalist'>{vocal_opts}</select></label>
    </div>
    <button type='submit'>Generate MIDI Pack</button>
  </form>
  <div class='panels'>
    <div class='panel'><h3>What changed in V1.1</h3><ul>
      <li>Real section starts: no more everything from bar 1</li>
      ...
      <li>Aligned full-length stem export for Logic/Ableton</li>
    </ul></div>
    <div class='panel'><h3>Included stems</h3><ul>{stem_items}</ul></div>
  </div>
</div></body></html>"""
...
def section_type(name: str):
    ...
    if 'breakdown' in n: return 'breakdown'
